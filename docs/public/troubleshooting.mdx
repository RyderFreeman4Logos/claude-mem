...
**Stuck Detection**: Messages in `processing` state for >5 minutes are considered stuck. Claude-mem resets them to `pending` on startup and graceful shutdown, auto-recovers pending queues on startup, and the 5-minute orphan scanner retries any sessions stranded without a restart.
