...
**Symptoms**: Observations remain stuck in the processing queue, or the queue is not draining after a worker crash/restart even though the worker is running again.

**Background**: Worker restarts are safe as of PR #24 (commit `ce34b858`). Claude-mem resets stale `processing` messages back to `pending` on startup and graceful shutdown, auto-recovers pending queues on startup, and runs a 5-minute orphan scanner to catch stranded work. Use the tools below when you want to inspect queue health or trigger recovery immediately because the backlog still is not clearing.
...
**Stuck Detection**: Messages that remain in `processing` for >60 seconds are treated as stale by self-heal. Each stale recovery bumps `retry_count` and moves the message back to `pending`. After 10 stale recovery cycles, the message is permanently marked `failed` to prevent infinite loops. Startup/shutdown recovery and the 5-minute orphan scanner still reclaim stranded work after crashes or restarts.
...
CREATE TABLE pending_messages (
  ...
  retry_count INTEGER,             -- Retry counter for backoff and stale self-heal tracking
...
Messages that fail during processing are automatically retried with exponential backoff (30s, 60s, then 2 minutes max). There is no hard cap on retries for normal failure paths, so the worker keeps retrying until the message succeeds. If a message instead gets stuck in `status='processing'` for more than 60 seconds and self-heal keeps resetting it, it is gravestoned to `status='failed'` after 10 stale recovery cycles to prevent infinite loops. See `MAX_RETRY_BEFORE_GRAVESTONE` in `src/services/sqlite/PendingMessageStore.ts`.
