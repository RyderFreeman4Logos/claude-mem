...
description: "Inspect queue health and recover stuck observations when automatic recovery needs help"
---
...
Claude-mem includes automatic queue recovery, and this guide covers the manual tools you can use to inspect queue health or nudge observations that remain stuck after crashes, restarts, or unexpected shutdowns.

**Key Change since PR #24 (`ce34b858`)**: Worker restarts are safe. Claude-mem resets stale `processing` messages to `pending` on startup and graceful shutdown, automatically resumes pending queues on startup, and runs a 5-minute orphan scanner. Manual recovery is now a fallback for visibility, immediate operator control, or cases where the backlog still is not draining.

## When Do You Need Manual Recovery?

You should trigger manual recovery when:

- **Worker crashed or restarted and the queue still is not draining** - Startup recovery should handle most cases, but you may want to inspect or nudge it immediately
- **No new summaries appearing** - Observations are being saved but not processed into summaries
- **Stuck messages detected** - Messages showing as "processing" for >5 minutes and you want immediate visibility or intervention
- **System crashes** - Unexpected shutdowns left messages in incomplete states and you want to verify recovery progress
...
4. **failed** → Permanently marked failed after 10 stale recovery cycles

### Stuck Detection

Messages in `processing` state for **>60 seconds** are treated as stale by self-heal:

- Each stale recovery bumps `retry_count` and moves the message back to `pending`
- After 10 stale recovery cycles, the message is permanently marked `failed` to prevent infinite loops
- Startup/shutdown recovery and the 5-minute orphan scanner can also reclaim stranded work
- The CLI/API tools let you inspect status or trigger recovery immediately if needed
...
Messages only move to `failed` when they are explicitly marked failed during handling. Normal `markFailed()` retries use exponential backoff with no hard retry limit, while stale self-heal recovery only gravestones messages after 10 recovery cycles:
...
If you're upgrading from a build before PR #24 (`ce34b858`):

**Before PR #24**:
- Worker interruptions could leave `pending_messages` stranded mid-processing
- Restarting the worker was not a reliable recovery path
- Operators often had to inspect and repair the queue manually

**Current Behavior**:
- Worker restarts are safe
- Startup and graceful shutdown both reset stale `processing` rows back to `pending`
- Startup recovery resumes pending queues automatically
- A 5-minute orphan scanner catches stranded sessions without requiring a restart
- CLI and API recovery tools remain available for inspection and on-demand intervention

**Migration Steps**:
1. Upgrade to a build that includes PR #24 (`ce34b858`)
2. Restart the worker if needed
3. Check queue status: `bun scripts/check-pending-queue.ts`
4. Process manually only if the backlog still needs intervention: `bun scripts/check-pending-queue.ts --process`
