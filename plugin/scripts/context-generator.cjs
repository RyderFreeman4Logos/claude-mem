...
`)[2]||"").match(/at\s+(?:.*\s+)?\(?([^:]+):(\d+):(\d+)\)?/),m=p?`${p[1].split("/").pop()}:${p[2]}`:"unknown",u={...s,location:m};return this.warn(e,`[HAPPY-PATH] ${t}`,u,n),o}},l=new H;var Ct={};function bt(){return typeof __dirname<"u"?__dirname:(0,S.dirname)((0,pe.fileURLToPath)(Ct.url))}var Ot=bt(),N=y.get("CLAUDE_MEM_DATA_DIR"),W=process.env.CLAUDE_CONFIG_DIR||(0,S.join)((0,de.homedir)(),".claude"),Ht=(0,S.join)(N,"archives"),Wt=(0,S.join)(N,"logs"),Yt=(0,S.join)(N,"trash"),Vt=(0,S.join)(N,"backups"),qt=(0,S.join)(N,"modes"),Kt=(0,S.join)(N,"settings.json"),le=(0,S.join)(N,"claude-mem.db"),Jt=(0,S.join)(N,"vector-db"),zt=(0,S.join)(N,"observer-sessions"),Qt=(0,S.join)(W,"settings.json"),Zt=(0,S.join)(W,"commands"),es=(0,S.join)(W,"CLAUDE.md");function ue(r){(0,ce.mkdirSync)(r,{recursive:!0})}function me(){return(0,S.join)(Ot,"..")}var $=class{db;constructor(e=le){e!==":memory:"&&ue(N),this.db=new _e.Database(e),this.db.run("PRAGMA journal_mode = WAL"),this.db.run("PRAGMA synchronous = NORMAL"),this.db.run("PRAGMA foreign_keys = ON"),this.initializeSchema(),this.ensureWorkerPortColumn(),this.ensurePromptTrackingColumns(),this.removeSessionSummariesUniqueConstraint(),this.addObservationHierarchicalFields(),this.makeObservationsTextNullable(),this.createUserPromptsTable(),this.ensureDiscoveryTokensColumn(),this.createPendingMessagesTable(),this.renameSessionIdColumns(),this.repairSessionIdColumnRename(),this.addFailedAtEpochColumn()}initializeSchema(){this.db.run(`
    ...
    `).run(e,t,s.request,s.investigated,s.learned,s.completed,s.next_steps,s.notes,n||null,o,d,a);return{id:Number(m.lastInsertRowid),createdAtEpoch:a}}storeObservations(e,t,s,n,o,i=0,a){let d=a??Date.now(),p=new Date(d).toISOString();return this.db.transaction(()=>{let u=[],g=this.db.prepare(`
        ...
        (memory_session_id, project, type, title, subtitle, facts, narrative, concepts,
         files_read, files_modified, prompt_number, discovery_tokens, created_at, created_at_epoch)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `);for(let T of s){let b=this.checkObservationExists(e,T.title,T.subtitle,T.type);if(b){l.debug("DB","Observation already exists in batch, skipping",{obsId:b.id,title:T.title,subtitle:T.subtitle}),u.push(b.id);continue}let _=g.run(e,t,T.type,T.title,T.subtitle,JSON.stringify(T.facts),T.narrative,JSON.stringify(T.concepts),JSON.stringify(T.files_read),JSON.stringify(T.files_modified),o||null,i,p,d);u.push(Number(_.lastInsertRowid))}let E=null;if(n){let b=this.db.prepare(`
          INSERT INTO session_summaries
          (memory_session_id, project, request, investigated, learned, completed,
           next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
