...
`)[2]||"").match(/at\s+(?:.*\s+)?\(?([^:]+):(\d+):(\d+)\)?/),u=m?`${m[1].split("/").pop()}:${m[2]}`:"unknown",_={...s,location:u};return this.warn(e,`[HAPPY-PATH] ${t}`,_,n),o}},p=new W;var Rt={};function St(){return typeof __dirname<"u"?__dirname:(0,h.dirname)((0,de.fileURLToPath)(Rt.url))}var Ot=St(),C=I.get("CLAUDE_MEM_DATA_DIR"),y=process.env.CLAUDE_CONFIG_DIR||(0,h.join)((0,ie.homedir)(),".claude"),Ht=(0,h.join)(y,"plugins","marketplaces","thedotmack"),Bt=(0,h.join)(C,"archives"),Wt=(0,h.join)(C,"logs"),Yt=(0,h.join)(C,"trash"),qt=(0,h.join)(C,"backups"),Vt=(0,h.join)(C,"modes"),Kt=(0,h.join)(C,"settings.json"),ce=(0,h.join)(C,"claude-mem.db"),Jt=(0,h.join)(C,"vector-db"),Qt=(0,h.join)(C,"observer-sessions"),zt=(0,h.join)(y,"settings.json"),Zt=(0,h.join)(y,"commands"),es=(0,h.join)(y,"CLAUDE.md");function pe(r){(0,ae.mkdirSync)(r,{recursive:!0})}function me(){return(0,h.join)(Ot,"..")}var ue=require("crypto");var Nt=3e4;function _e(r,e,t){return(0,ue.createHash)("sha256").update((r||"")+(e||"")+(t||"")).digest("hex").slice(0,16)}function le(r,e,t){let s=t-Nt;return r.prepare("SELECT id, created_at_epoch FROM observations WHERE content_hash = ? AND created_at_epoch > ?").get(e,s)}var $=class{db;constructor(e=ce){e!==":memory:"&&pe(C),this.db=new Ee.Database(e),this.db.run("PRAGMA journal_mode = WAL"),this.db.run("PRAGMA synchronous = NORMAL"),this.db.run("PRAGMA foreign_keys = ON"),this.initializeSchema(),this.ensureWorkerPortColumn(),this.ensurePromptTrackingColumns(),this.removeSessionSummariesUniqueConstraint(),this.addObservationHierarchicalFields(),this.makeObservationsTextNullable(),this.createUserPromptsTable(),this.ensureDiscoveryTokensColumn(),this.createPendingMessagesTable(),this.renameSessionIdColumns(),this.repairSessionIdColumnRename(),this.addFailedAtEpochColumn(),this.addOnUpdateCascadeToForeignKeys(),this.addObservationContentHashColumn(),this.addSessionCustomTitleColumn()}initializeSchema(){this.db.run(`
    ...
    `).all(e)}getObservationById(e){return this.db.prepare(`
      ...
      FROM observations
      WHERE id = ?
    `).get(e)||null}getObservationsByIds(e,t={}){if(e.length===0)return[];let{orderBy:s="date_desc",limit:n,project:o,type:i,concepts:a,files:c}=t,m=s==="date_asc"?"ASC":"DESC",u=n?`LIMIT ${n}`:"",_=e.map(()=>"?").join(","),T=[...e],E=[];if(o&&(E.push("project = ?"),T.push(o)),i)if(Array.isArray(i)){let l=i.map(()=>"?").join(",");E.push(`type IN (${l})`),T.push(...i)}else E.push("type = ?"),T.push(i);if(a){let l=Array.isArray(a)?a:[a],b=l.map(()=>"EXISTS (SELECT 1 FROM json_each(concepts) WHERE value = ?)");T.push(...l),E.push(`(${b.join(" OR ")})`)}if(c){let l=Array.isArray(c)?c:[c],b=l.map(()=>"(EXISTS (SELECT 1 FROM json_each(files_read) WHERE value LIKE ?) OR EXISTS (SELECT 1 FROM json_each(files_modified) WHERE value LIKE ?))");l.forEach(f=>{T.push(`%${f}%`,`%${f}%`)}),E.push(`(${b.join(" OR ")})`)}let g=E.length>0?`WHERE id IN (${_}) AND ${E.join(" AND ")}`:`WHERE id IN (${_})`;return this.db.prepare(`
      SELECT *
      FROM observations
      ${g}
    ...
    `).get(e);return a?(t&&this.db.prepare(`
      ...
      VALUES (?, NULL, ?, ?, ?, ?, ?, 'active')
    `).run(e,t,s,n||null,o.toISOString(),i),this.db.prepare("SELECT id FROM sdk_sessions WHERE content_session_id = ?").get(e).id)}markSessionCompleted(e){let t=new Date,s=t.getTime();return this.db.transaction(o=>{let i=this.db.prepare(`
      ...
      `).get(o);return i.count===0?(this.db.prepare(`
          ...
          SET status = 'completed',
              ...
              completed_at_epoch = ?
          WHERE id = ?
        `).run(t.toISOString(),s,o),p.debug("DB","Session marked as completed",{sessionDbId:o}),!0):(p.warn("DB","Attempted to mark session completed but queue not empty",{sessionDbId:o,pendingCount:i.count}),!1)})(e)}saveUserPrompt(e,t,s){let n=new Date,o=n.getTime();return this.db.prepare(`
    ...
    `).get(e,t)?.prompt_text??null}checkObservationExists(e,t,s,n){return this.db.prepare(`
      ...
      WHERE memory_session_id = ?
        ...
        AND type = ?
    `).get(e,t,t,s,s,n)}storeObservation(e,t,s,n,o=0,i){let a=i??Date.now(),c=new Date(a).toISOString(),m=_e(e,s.title,s.narrative),u=le(this.db,m,a);if(u)return{id:u.id,createdAtEpoch:u.created_at_epoch};let T=this.db.prepare(`
      INSERT INTO observations
      (memory_session_id, project, type, title, subtitle, facts, narrative, concepts,
       files_read, files_modified, prompt_number, discovery_tokens, content_hash, created_at, created_at_epoch)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(e,t,s.type,s.title,s.subtitle,JSON.stringify(s.facts),s.narrative,JSON.stringify(s.concepts),JSON.stringify(s.files_read),JSON.stringify(s.files_modified),n||null,o,m,c,a);return{id:Number(T.lastInsertRowid),createdAtEpoch:a}}storeSummary(e,t,s,n,o=0,i){let a=i??Date.now(),c=new Date(a).toISOString(),u=this.db.prepare(`
      INSERT INTO session_summaries
      (memory_session_id, project, request, investigated, learned, completed,
       next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(e,t,s.request,s.investigated,s.learned,s.completed,s.next_steps,s.notes,n||null,o,c,a);return{id:Number(u.lastInsertRowid),createdAtEpoch:a}}storeObservations(e,t,s,n,o,i=0,a){let c=a??Date.now(),m=new Date(c).toISOString();return this.db.transaction(()=>{let _=[],T=this.db.prepare(`
        INSERT INTO observations
        (memory_session_id, project, type, title, subtitle, facts, narrative, concepts,
         files_read, files_modified, prompt_number, discovery_tokens, created_at, created_at_epoch)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `);for(let g of s){let S=this.checkObservationExists(e,g.title,g.subtitle,g.type);if(S){p.debug("DB","Observation already exists in batch, skipping",{obsId:S.id,title:g.title,subtitle:g.subtitle}),_.push(S.id);continue}let l=T.run(e,t,g.type,g.title,g.subtitle,JSON.stringify(g.facts),g.narrative,JSON.stringify(g.concepts),JSON.stringify(g.files_read),JSON.stringify(g.files_modified),o||null,i,m,c);_.push(Number(l.lastInsertRowid))}let E=null;if(n){let S=this.db.prepare(`
          INSERT INTO session_summaries
          (memory_session_id, project, request, investigated, learned, completed,
           next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).run(e,t,n.request,n.investigated,n.learned,n.completed,n.next_steps,n.notes,o||null,i,m,c);E=Number(S.lastInsertRowid)}return{observationIds:_,summaryId:E,createdAtEpoch:c}})()}storeObservationsAndMarkComplete(e,t,s,n,o,i,a,c=0,m){let u=m??Date.now(),_=new Date(u).toISOString();return this.db.transaction(()=>{let E=[],g=this.db.prepare(`
        INSERT INTO observations
        (memory_session_id, project, type, title, subtitle, facts, narrative, concepts,
         files_read, files_modified, prompt_number, discovery_tokens, created_at, created_at_epoch)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `);for(let b of s){let f=g.run(e,t,b.type,b.title,b.subtitle,JSON.stringify(b.facts),b.narrative,JSON.stringify(b.concepts),JSON.stringify(b.files_read),JSON.stringify(b.files_modified),a||null,c,_,u);E.push(Number(f.lastInsertRowid))}let S;if(n){let f=this.db.prepare(`
          INSERT INTO session_summaries
          (memory_session_id, project, request, investigated, learned, completed,
           next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).run(e,t,n.request,n.investigated,n.learned,n.completed,n.next_steps,n.notes,a||null,c,_,u);S=Number(f.lastInsertRowid)}return this.db.prepare(`
      ...
      `).run(u,o),{observationIds:E,summaryId:S,createdAtEpoch:u}})()}getSessionSummariesByIds(e,t={}){if(e.length===0)return[];let{orderBy:s="date_desc",limit:n,project:o}=t,i=s==="date_asc"?"ASC":"DESC",a=n?`LIMIT ${n}`:"",c=e.map(()=>"?").join(","),m=[...e],u=o?`WHERE id IN (${c}) AND project = ?`:`WHERE id IN (${c})`;return o&&m.push(o),this.db.prepare(`
      SELECT * FROM session_summaries
      ${u}
      ORDER BY created_at_epoch ${i}
      ${a}
    `).all(...m)}getUserPromptsByIds(e,t={}){if(e.length===0)return[];let{orderBy:s="date_desc",limit:n,project:o}=t,i=s==="date_asc"?"ASC":"DESC",a=n?`LIMIT ${n}`:"",c=e.map(()=>"?").join(","),m=[...e],u=o?"AND s.project = ?":"";return o&&m.push(o),this.db.prepare(`
      SELECT
        up.*,
        s.project,
        s.memory_session_id
      ...
      ${a}
    `).all(...m)}getTimelineAroundTimestamp(e,t=10,s=10,n){return this.getTimelineAroundObservation(null,e,t,s,n)}getTimelineAroundObservation(e,t,s=10,n=10,o){let i=o?"AND project = ?":"",a=o?[o]:[],c,m;if(e!==null){let l=`
        SELECT id, created_at_epoch
        FROM observations
        WHERE id <= ? ${i}
        ORDER BY id DESC
      ...
      `,b=`
        ...
        ORDER BY id ASC
        LIMIT ?
      `;try{let f=this.db.prepare(l).all(e,...a,s+1),N=this.db.prepare(b).all(e,...a,n+1);if(f.length===0&&N.length===0)return{observations:[],sessions:[],prompts:[]};c=f.length>0?f[f.length-1].created_at_epoch:t,m=N.length>0?N[N.length-1].created_at_epoch:t}catch(f){return p.error("DB","Error getting boundary observations",void 0,{error:f,project:o}),{observations:[],sessions:[],prompts:[]}}}else{let l=`
      ...
      `,b=`
        ...
        ORDER BY created_at_epoch ASC
        LIMIT ?
      `;try{let f=this.db.prepare(l).all(t,...a,s),N=this.db.prepare(b).all(t,...a,n+1);if(f.length===0&&N.length===0)return{observations:[],sessions:[],prompts:[]};c=f.length>0?f[f.length-1].created_at_epoch:t,m=N.length>0?N[N.length-1].created_at_epoch:t}catch(f){return p.error("DB","Error getting boundary timestamps",void 0,{error:f,project:o}),{observations:[],sessions:[],prompts:[]}}}let u=`
      SELECT *
      FROM observations
...
`):null}function Lt(r,e,t,s,n,o){let i=[];o?i.push(...Ye(r)):i.push(...De(r));let a=null,c="",m=!1;for(let u of e)if(u.type==="summary"){m&&(i.push(""),m=!1,a=null,c="");let _=u.data,T=st(_.displayTime);o?i.push(...Je(_,T)):i.push(...we(_,T))}else{let _=u.data,T=ot(_.files_modified,n,_.files_read),E=rt(_.created_at),g=E!==c,S=g?E:"";c=E;let l=t.has(_.id);if(T!==a&&(m&&i.push(""),o?i.push(...qe(T)):i.push(...xe(T)),a=T,m=!0),l){let b=vt(_,s);o?i.push(...Ke(_,E,g,b,s)):(m&&!o&&(i.push(""),m=!1),i.push(...ke(_,S,b,s)),a=null)}else o?i.push(Ve(_,E,g,s)):i.push(Ue(_,S,s))}return m&&i.push(""),i}function it(r,e,t,s,n){let o=[],i=yt(r);for(let[a,c]of i)o.push(...Lt(a,c,e,t,s,n));return o}function at(r,e,t){return!(!r.showLastSummary||!e||!!!(e.investigated||e.learned||e.completed||e.next_steps)||t&&e.created_at_epoch<=t.created_at_epoch)}function dt(r,e){let t=[];return e?(t.push(...U("Investigated",r.investigated,d.blue)),t.push(...U("Learned",r.learned,d.yellow)),t.push(...U("Completed",r.completed,d.green)),t.push(...U("Next Steps",r.next_steps,d.magenta))):(t.push(...x("Investigated",r.investigated)),t.push(...x("Learned",r.learned)),t.push(...x("Completed",r.completed)),t.push(...x("Next Steps",r.next_steps))),t}function ct(r,e){return e?Qe(r):$e(r)}function pt(r,e,t){return!P(e)||r.totalDiscoveryTokens<=0||r.savings<=0?[]:t?ze(r.totalDiscoveryTokens,r.totalReadTokens):Fe(r.totalDiscoveryTokens,r.totalReadTokens)}var Mt=mt.default.join((0,ut.homedir)(),".claude","plugins","marketplaces","thedotmack","plugin",".install-version");function Dt(){try{return new $}catch(r){if(r.code==="ERR_DLOPEN_FAILED"){try{(0,_t.unlinkSync)(Mt)}catch(e){p.debug("SYSTEM","Marker file cleanup failed (may not exist)",{},e)}return p.error("SYSTEM","Native module rebuild needed - restart Claude Code to auto-fix"),null}throw r}}function xt(r,e){return e?Ze(r):Pe(r)}function Ut(r,e,t,s,n,o,i){let a=[],c=K(e);a.push(...et(r,c,s,i));let m=t.slice(0,s.sessionCount),u=Ne(m,t),_=Z(e,u),T=Ce(e,s.fullObservationCount);a.push(...it(_,T,s,n,i));let E=t[0],g=e[0];at(s,E,g)&&a.push(...dt(E,i));let S=z(e,s,o,n);return a.push(...ct(S,i)),a.push(...pt(c,s,i)),a.join(`
`).trimEnd()}async function te(r,e=!1){let t=Y(),s=r?.cwd??process.cwd(),n=ge(s),o=r?.projects||[n],i=Dt();if(!i)return"";try{let a=o.length>1?Oe(i,o,t):J(i,n,t),c=o.length>1?Re(i,o,t):Q(i,n,t);return a.length===0&&c.length===0?xt(n,e):Ut(n,a,c,t,s,r?.session_id,e)}finally{i.close()}}0&&(module.exports={generateContext});
