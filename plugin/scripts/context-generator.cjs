...
`+JSON.stringify(o,null,2):m=" "+this.formatData(o));let u="";if(n){let{sessionId:E,memorySessionId:T,correlationId:S,..._}=n;Object.keys(_).length>0&&(u=` {${Object.entries(_).map(([f,R])=>`${f}=${R}`).join(", ")}}`)}let g=`[${i}] [${a}] [${d}] ${p}${s}${u}${m}`;if(this.logFilePath)try{(0,I.appendFileSync)(this.logFilePath,g+`
...
`)[2]||"").match(/at\s+(?:.*\s+)?\(?([^:]+):(\d+):(\d+)\)?/),m=p?`${p[1].split("/").pop()}:${p[2]}`:"unknown",u={...s,location:m};return this.warn(e,`[HAPPY-PATH] ${t}`,u,n),o}},l=new G;var Ct={};function bt(){return typeof __dirname<"u"?__dirname:(0,b.dirname)((0,pe.fileURLToPath)(Ct.url))}var Ot=bt(),N=y.get("CLAUDE_MEM_DATA_DIR"),H=process.env.CLAUDE_CONFIG_DIR||(0,b.join)((0,de.homedir)(),".claude"),Gt=(0,b.join)(N,"archives"),Ht=(0,b.join)(N,"logs"),Yt=(0,b.join)(N,"trash"),Vt=(0,b.join)(N,"backups"),qt=(0,b.join)(N,"modes"),Kt=(0,b.join)(N,"settings.json"),le=(0,b.join)(N,"claude-mem.db"),Jt=(0,b.join)(N,"vector-db"),zt=(0,b.join)(H,"settings.json"),Qt=(0,b.join)(H,"commands"),Zt=(0,b.join)(H,"CLAUDE.md");function ue(r){(0,ce.mkdirSync)(r,{recursive:!0})}function me(){return(0,b.join)(Ot,"..")}var $=class{db;constructor(e=le){e!==":memory:"&&ue(N),this.db=new _e.Database(e),this.db.run("PRAGMA journal_mode = WAL"),this.db.run("PRAGMA synchronous = NORMAL"),this.db.run("PRAGMA foreign_keys = ON"),this.initializeSchema(),this.ensureWorkerPortColumn(),this.ensurePromptTrackingColumns(),this.removeSessionSummariesUniqueConstraint(),this.addObservationHierarchicalFields(),this.makeObservationsTextNullable(),this.createUserPromptsTable(),this.ensureDiscoveryTokensColumn(),this.createPendingMessagesTable(),this.renameSessionIdColumns(),this.repairSessionIdColumnRename(),this.addFailedAtEpochColumn()}initializeSchema(){this.db.run(`
    ...
    `).run(e,t,s,n.toISOString(),o),this.db.prepare("SELECT id FROM sdk_sessions WHERE content_session_id = ?").get(e).id}markSessionCompleted(e){let t=new Date,s=t.getTime();return this.db.transaction(o=>{let i=this.db.prepare(`
      ...
      `).get(o);return i.count===0?(this.db.prepare(`
          ...
          SET status = 'completed',
              ...
              completed_at_epoch = ?
          WHERE id = ?
        `).run(t.toISOString(),s,o),l.debug("DB","Session marked as completed",{sessionDbId:o}),!0):(l.warn("DB","Attempted to mark session completed but queue not empty",{sessionDbId:o,pendingCount:i.count}),!1)})(e)}saveUserPrompt(e,t,s){let n=new Date,o=n.getTime();return this.db.prepare(`
    ...
    `).get(e,t)?.prompt_text??null}checkObservationExists(e,t,s,n){return this.db.prepare(`
      ...
      WHERE memory_session_id = ?
        ...
        AND type = ?
    `).get(e,t,t,s,s,n)}storeObservation(e,t,s,n,o=0,i){let a=this.checkObservationExists(e,s.title,s.subtitle,s.type);if(a)return l.debug("DB","Observation already exists, skipping",{obsId:a.id,title:s.title}),{id:a.id,createdAtEpoch:a.created_at_epoch};let d=i??Date.now(),p=new Date(d).toISOString(),u=this.db.prepare(`
      INSERT INTO observations
      (memory_session_id, project, type, title, subtitle, facts, narrative, concepts,
       files_read, files_modified, prompt_number, discovery_tokens, created_at, created_at_epoch)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(e,t,s.type,s.title,s.subtitle,JSON.stringify(s.facts),s.narrative,JSON.stringify(s.concepts),JSON.stringify(s.files_read),JSON.stringify(s.files_modified),n||null,o,p,d);return{id:Number(u.lastInsertRowid),createdAtEpoch:d}}storeSummary(e,t,s,n,o=0,i){let a=i??Date.now(),d=new Date(a).toISOString(),m=this.db.prepare(`
    ...
    `).run(e,t,s.request,s.investigated,s.learned,s.completed,s.next_steps,s.notes,n||null,o,d,a);return{id:Number(m.lastInsertRowid),createdAtEpoch:a}}storeObservations(e,t,s,n,o,i=0,a){let d=a??Date.now(),p=new Date(d).toISOString();return this.db.transaction(()=>{let u=[],g=this.db.prepare(`
        ...
        (memory_session_id, project, type, title, subtitle, facts, narrative, concepts,
         files_read, files_modified, prompt_number, discovery_tokens, created_at, created_at_epoch)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `);for(let T of s){let S=this.checkObservationExists(e,T.title,T.subtitle,T.type);if(S){l.debug("DB","Observation already exists in batch, skipping",{obsId:S.id,title:T.title,subtitle:T.subtitle}),u.push(S.id);continue}let _=g.run(e,t,T.type,T.title,T.subtitle,JSON.stringify(T.facts),T.narrative,JSON.stringify(T.concepts),JSON.stringify(T.files_read),JSON.stringify(T.files_modified),o||null,i,p,d);u.push(Number(_.lastInsertRowid))}let E=null;if(n){let S=this.db.prepare(`
          INSERT INTO session_summaries
          (memory_session_id, project, request, investigated, learned, completed,
           next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).run(e,t,n.request,n.investigated,n.learned,n.completed,n.next_steps,n.notes,o||null,i,p,d);E=Number(S.lastInsertRowid)}return{observationIds:u,summaryId:E,createdAtEpoch:d}})()}storeObservationsAndMarkComplete(e,t,s,n,o,i,a,d=0,p){let m=p??Date.now(),u=new Date(m).toISOString();return this.db.transaction(()=>{let E=[],T=this.db.prepare(`
        INSERT INTO observations
        (memory_session_id, project, type, title, subtitle, facts, narrative, concepts,
         files_read, files_modified, prompt_number, discovery_tokens, created_at, created_at_epoch)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `);for(let h of s){let f=T.run(e,t,h.type,h.title,h.subtitle,JSON.stringify(h.facts),h.narrative,JSON.stringify(h.concepts),JSON.stringify(h.files_read),JSON.stringify(h.files_modified),a||null,d,u,m);E.push(Number(f.lastInsertRowid))}let S;if(n){let f=this.db.prepare(`
          INSERT INTO session_summaries
          (memory_session_id, project, request, investigated, learned, completed,
           next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
